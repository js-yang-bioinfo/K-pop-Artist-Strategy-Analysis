...
# 필요한 열 선택
data = data[['artist', 'average_sales_amount']]
dartist = ['NCT DREAM', 'NewJeans', 'Stray Kids (스트레이 키즈)', 'V', 'ZEROBASEONE (제로베이스원)', '세븐틴', '정국', '지민']

# 특정 아티스트 제거
data = data[~data['artist'].isin(dartist)]
...
# greater_than_one = data[data_scaled[:, 0] > 2.2]['artist'].tolist()
# less_than_one = data[data_scaled[:, 0] <= 2.2]['artist'].tolist()

# # 새로운 데이터프레임 생성
# comparison_df = pd.DataFrame({'앨범 1단계': pd.Series(less_than_one), '앨범 2단계': pd.Series(greater_than_one)})

# 결과 데이터프레임 저장
# output_file_path = r'C:\Users\user\Desktop\Dscover_study\Main_project\Yealy Sales_amount\Visualization.xlsx'
